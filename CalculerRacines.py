...
def afficher():
        cadre.place(relx=0.9, rely=0.25, anchor="center")
        switch_delta_neg.pack(ipady=10, ipadx=5, anchor="w")
        switch_factoriser.pack(ipady=10, ipadx=5, anchor="w")
def masquer() :
        cadre.place_forget()
        switch_delta_neg.pack_forget()
...
def HISTT():
    ...
    if df==True:
         ...
         hj = CTkScrollableFrame(app, width= 150, height=10)
...
def key_pressed(event):
    if event.keysym == 'm' and event.state == 4:
     if main==True :
        main1()
